...
double average(const initializer_list<int> & data) {
    ...
    avg = accumulate(data.begin(), data.end(),0.)/data.size();
...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    ...
    else return data.cend();// tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int main() {
    ...
    try {
        cout << average({1,2,3,4,4,4,4}) << endl;
    } catch (exception& e) {
        cout <<e.what() <<endl;
    }
    
    list<int> test = { 100, 90, 91, 92, 93, 53, 54, 55, 56, 57, 91, 92 };
    list<int> test2 = { 100, 90, 91, 92, 93, 53, 54, 55, 56, 57, 91, 92 };
//    cout << *findValue(test, 4) << endl;
    
//    auto i = findInRange200to400(test);
//    if (i == test.cend()) {
//        cout << "Nenašiel sa" << endl;
//    }
//    else cout << *i << endl;
    
//    replace200to400by5(test);
//    incrementAll(test);
//    helpAfterExam1(test);
//    helpAfterExam2(test);
//    list<int> test3 = add(test, test2);
//
//    vector<int> vect = {2,5,1,2,9,10};
//    sort1(vect);
//    sort2(vect);
